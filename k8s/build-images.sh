#!/usr/bin/env bash
# Builds the four service images with local tags (see build-images.ps1 for notes).
set -euo pipefail
...
docker build -t fcg-notifications-api:local "$ROOT/fiap-cloud-games-notifications-api"
echo "Done: fcg-{users,catalog,payments,notifications}-api:local"
