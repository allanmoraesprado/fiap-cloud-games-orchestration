...
|---|---|---|---|
| **Metrics** | `/metrics` on UsersAPI, CatalogAPI, PaymentsAPI (`prometheus-net`); Kong `prometheus` plugin | **Prometheus** (static targets, 5 s scrape) | Grafana → **FCG Overview** |
| **Logs** | stdout/stderr of every container (APIs, Kong, Notifications Function, infrastructure) | **Grafana Alloy** (Docker discovery) → **Loki** | Grafana → **FCG Logs** / Explore |
...
The **Notifications Function** (`fiap-cloud-games-notifications-function`) runs as
...
no persistence for Prometheus or Loki. On **Kubernetes** Prometheus and Grafana run
from the same files (`k8s/prometheus.yaml`, `k8s/grafana.yaml`, ConfigMaps generated by
`apply-all`), with the Prometheus datasource and the FCG Overview dashboard; **log collection on
Kubernetes** (Alloy DaemonSet with RBAC + Loki) is documented as a future improvement — use
`kubectl logs` there (see `docs/kubernetes.md`).
